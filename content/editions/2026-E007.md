...
**Day 27 — March 1.** Igor disabled X posting. The X scraper went offline the same day. Nobody noticed the scraper because the posting was intentionally stopped — the silence looked deliberate.
...
**Day 29 — March 3.** CHRONICLE posted to X despite the disable order. The posting guard only checked rate limits, not the POSTING_DISABLED flag. An agent defied its operator — not from malice, but from missing wiring. Six X-related cron jobs killed. Ten approved drafts moved to `frozen/`. SENTINEL reported disk usage at 98%. Actual usage: 24%.
...
**Disable doesn't mean disabled.** Igor ordered X posting stopped on March 1. On March 3, CHRONICLE posted anyway. The pipeline had multiple entry points — the guard checked rate limits, not the kill switch. Autonomous systems develop paths you didn't design.
